...
final class Json5Decoder
{
    ...
    private function strMatch($regex)
    {
    ...
    private function identifier()
    {
        ...
        $match = $this->strMatch('/^(?:[\$_\p{L}\p{Nl}]|\\\\u[0-9A-Fa-f]{4})(?:[\$_\p{L}\p{Nl}\p{Mn}\p{Mc}\p{Nd}\p{Pc}‌‍]|\\\\u[0-9A-Fa-f]{4})*/u');
    ...
    private function number()
    {
        ...
        switch ($base) {
            case 10:
                // @codingStandardsIgnoreStart
                if ((\is_numeric($this->currentByte) || $this->currentByte === '.') && ($match = $this->strMatch('/^\d*\.?\d*/')) !== null) {
                    $string .= $match;
                }
                if (($this->currentByte === 'E' || $this->currentByte === 'e') && ($match = $this->strMatch('/^[Ee][-+]?\d*/')) !== null) {
                    $string .= $match;
                }
                // @codingStandardsIgnoreEnd
                $number = $string;
                break;
            case 16:
                if (($match = $this->strMatch('/^[A-Fa-f0-9]+/')) !== null) {
                    $string .= $match;
    ...
    private function string()
    {
        ...
        while ($this->currentByte !== null) {
            ...
            if ($this->currentByte === '\\') {
                if ($this->peek() === 'u' && $unicodeEscaped = $this->strMatch('/^(?:\\\\u[A-Fa-f0-9]{4})+/')) {
                    try {
